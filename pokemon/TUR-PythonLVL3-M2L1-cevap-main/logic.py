...
class Pokemon:
    ...
    def __init__(self, pokemon_trainer):
        ...
        async def feed(self, feed_interval = 60, hp_increase = 10):
            ...
            elapsed_time = (current_time - self.last_feed_time).total_seconds()
            if elapsed_time >= feed_interval:
                self.hp += hp_increase
                self.last_feed_time = current_time
                return f"Pokémon eğitmeni @{self.pokemon_trainer} Pokémonunu besledi! Sağlık durumu şimdi {self.hp}"
            else:
                remaining_time = feed_interval - elapsed_time
                return f"Pokémon eğitmeni @{self.pokemon_trainer} Pokémonunu besleyemez. Lütfen {int(remaining_time)} saniye bekleyin."
